...
int calib_SetBackEndOffset(rp_channel_t channel) {
   ...
   // bhaskarm - Disabling generator code till the quad_generate is stable
    ...
    //ECHECK(prec_GenWaveform(channel, RP_WAVEFORM_SINE));
    //ECHECK(prec_GenAmp(channel, 0));
    //ECHECK(prec_GenOffset(channel, 0));
    //ECHECK(prec_GenOutEnable(channel));
...
int calib_SetBackEndScale(rp_channel_t channel) {
   ...
   // bhaskarm - Disabling generator code till the quad_generate is stable
    //ECHECK(prec_GenReset());
    //ECHECK(prec_GenWaveform(channel, RP_WAVEFORM_PWM));
    //ECHECK(prec_GenDutyCycle(channel, 1));
    //ECHECK(prec_GenAmp(channel, CONSTANT_SIGNAL_AMPLITUDE));
    //ECHECK(prec_GenOffset(channel, 0));
    //ECHECK(prec_GenOutEnable(channel));
...
static int getGenAmp(rp_channel_t channel, float amp, float* min, float* max) {
   // bhaskarm - Disabling generator code till the quad_generate is stable
    //ECHECK(prec_GenReset());
    //ECHECK(prec_GenWaveform(channel, RP_WAVEFORM_SINE));
    //ECHECK(prec_GenAmp(channel, amp));
    //ECHECK(prec_GenOffset(channel, 0));
    //ECHECK(prec_GenOutEnable(channel));
...
static int getGenDC_int(rp_channel_t channel, float dc) {
   // bhaskarm - Disabling generator code till the quad_generate is stable
    //ECHECK(prec_GenReset());
    //ECHECK(prec_GenWaveform(channel, RP_WAVEFORM_DC));
    //ECHECK(prec_GenAmp(channel, 0));
    //ECHECK(prec_GenOffset(channel, dc));
    //ECHECK(prec_GenOutEnable(channel));
